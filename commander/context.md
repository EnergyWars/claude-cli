...
- **Navigation (`ui/navigation/AppNavHost.kt`, `Routes`):** Linearer Fluss ohne Hamburger-Menü. `NoConnection → SETUP`, `NeedsLogin → LOGIN`, `Ready` ohne gemerktes Projekt `→ PROJECT_SELECT` (reine Namensliste aus `manifest.paths`, Tap wählt + persistiert), `Ready` mit gemerktem Projekt `→ PROJECT_HOME` (Hub: Projekt-Dropdown zum Wechseln oben, Zahnrad zu Settings, darunter sieben Einträge: **Befehle**, **Downloads**, **Dev-Agent**, **Tickets**, **Verlauf**, **Feedback**, **Sammlung**). Jeder Bereich ist ein eigener, fokussierter Screen ohne Querverweise auf die anderen:
  ...
  - `TICKETS`/`TICKET_DETAIL` (`ui/tickets/`): wie zuvor, aber `pathName` ist jetzt **Pflichtparameter** – der frühere globale, pfadlose Modus (alle Pfade gemischt, Pfad-Dropdown beim Anlegen) wurde entfernt, da nicht mehr erreichbar. `TICKET_DETAIL` hat zusätzlich einen Play-Button (Bestätigungsdialog inkl. Agenten-Auswahl aus `manifest.agents`, Default `cl dev` falls vorhanden) – schickt `claudeInstruction` per `POST /<agent>` los, navigiert danach zu `COMMAND_DETAIL` und schließt das Ticket (`PATCH status=done`) erst nach erfolgreichem Start des Commands.
  - `HISTORY` (`ui/history/HistoryScreen.kt`): Liste aus `GET /commands/<pathName>` (neueste zuerst), pollt alle 3s (`HistoryViewModel`). Tap öffnet dieselbe `COMMAND_DETAIL`-Seite wie „Befehle"/„Dev-Agent". `ui/history/TimestampFormat.kt` formatiert `createdAt` (reine Funktion).
  - `FEEDBACK` (`ui/feedback/FeedbackListScreen.kt`, **ohne** `pathName` – serverweit, nicht projektgebunden): Liste aus `GET /feedback` (authentifiziert), pro Eintrag Bearbeiten (Inline-`AppTextField` + Speichern → `PATCH /feedback/:id`), Löschen (`AppConfirmDialog` → `DELETE /feedback/:id`) und "In Ticket umwandeln" (Dialog mit Projekt-Dropdown aus `manifest.paths`, ruft `createTicket(pathName, text)` gefolgt von `deleteFeedback(id)`). Kein Erstellen aus der App heraus – `POST /feedback` ist für externe, unauthentifizierte Absender gedacht (z. B. `app-getter`-Nutzer oder ein Web-Formular).
...
Siehe `FEATURES.md`. Vollständig umgesetzt: Verbindungsaufbau/Pairing/Login, Projektauswahl + gemerktes Projekt, Projekt-Hub (Befehle/Downloads/Dev-Agent/Tickets/Verlauf/Feedback/Sammlung), Befehle ausführen, Downloads inkl. automatischem APK-Install, Dev-Agent-Lauf mit optionalem Kontext-Preset und Model-Wahl, Dev-Kontexte verwalten (Settings), Live-Command-Status, Command-Verlauf pro Projekt (live gepollt), Tickets (projektgebunden) inkl. Play-Button (Bestätigung + Agentenwahl, schließt das Ticket und startet den Agenten), Feedback verwalten (serverweit: bearbeiten/löschen/in Ticket umwandeln), Sammlung auslösen (`POST /collect`), Theme-Einstellungen + Verbindung trennen.
