...
Liste der `path.hosted`-Einträge des aktuellen Projekts. `type: "file"`-Einträge haben einen Download-/Install-Button und zeigen darunter gut sichtbar „Gebaut am <dd.mm.yyyy hh:mm>" (Zeitpunkt der letzten Änderung der Datei, in die Zeitzone des Geräts umgerechnet); `type: "path"`-Einträge lassen sich aufklappen, jede enthaltene Datei zeigt ebenfalls ihr eigenes „Gebaut am"-Label und lässt sich einzeln herunterladen. Dieselbe „Gebaut am"-Anzeige erscheint auch bei den Schnellzugriff-Downloads auf der Command-Detailseite (siehe „Status/Verlauf"). Endet der heruntergeladene Dateiname auf `.apk`, erscheint nach Abschluss ein Dialog mit drei Aktionen: „Installieren" (System-Install-Dialog inkl. einmaligem „Unbekannte Quellen erlauben", falls noch nicht freigegeben), „Teilen" (`ACTION_SEND`-Chooser, um die APK z. B. per Messenger/Mail/Cloud-Speicher weiterzugeben) oder „Löschen" (Bestätigungsnachfrage, entfernt die Datei vom Gerät). Tippen außerhalb des Dialogs bricht ohne Aktion ab, ohne die Datei zu löschen. Alle anderen Dateien werden wie bisher direkt zum Öffnen/Teilen angeboten (kein Dialog, keine Zwischenspeicherung). Downloads landen unter `getExternalFilesDir(DIRECTORY_DOWNLOADS)` (kein Speicher-Runtime-Permission nötig).

Eine fertig heruntergeladene, noch nicht gelöschte APK bleibt dauerhaft gespeichert. Beim nächsten Öffnen der Liste zeigt die betroffene Zeile statt des Download- ein Install-Icon; ein Tap darauf prüft zuerst direkt beim Server, ob sich die Datei seit dem Download geändert hat (aktueller Timestamp aus `GET /manifest` bzw. `GET /files/<pathName>/<hostedName>`), bevor irgendetwas installiert wird. Ist der Timestamp unverändert, öffnet sich der Installieren/Teilen/Löschen-Dialog sofort, ohne erneut herunterzuladen. Hat sich die Datei auf dem Server geändert, wird automatisch neu heruntergeladen und als zusätzliche Version in den Download-Verlauf (siehe unten) aufgenommen, statt die vorherige Version zu löschen. Der Dialog öffnet sich dabei **nicht** von selbst (weder beim Laden der Liste noch beim App-Neustart) – nur ein bewusster Tap auf das Install-/Download-Icon öffnet ihn. Erst „Löschen" entfernt die zuletzt heruntergeladene Datei endgültig vom Gerät (inkl. Eintrag im Download-Verlauf); existiert die zwischengespeicherte Datei nicht mehr (z. B. manuell gelöscht), zeigt die Zeile wieder das Download-Icon und ein Tap lädt neu herunter.

**Download-Verlauf:** Der Button „Download-Verlauf" oben auf dem Downloads-Screen öffnet eine eigene Seite mit allen Dateien, die für das aktuelle Projekt jemals heruntergeladen wurden (nicht nur APKs), gruppiert nach Datei. Pro Datei werden die **letzten drei heruntergeladenen Versionen** aufgehoben (älteste wird beim Herunterladen einer vierten Version automatisch von der Platte gelöscht), jede Zeile zeigt „Gebaut am" (Server-Zeitstempel, falls bekannt) sowie „Heruntergeladen am" (Geräte-Zeitpunkt des Downloads) und bietet zwei Aktionen: Öffnen (installiert eine `.apk` bzw. öffnet/teilt jede andere Datei über den System-Chooser) und Löschen (Bestätigungsnachfrage, entfernt genau diese eine Version dauerhaft vom Gerät, um Speicherplatz freizugeben). Ohne Verlauf zeigt die Seite den Hinweis „Kein Download-Verlauf vorhanden."
...
**Download läuft im Hintergrund weiter:** Ein gestarteter Download läuft unabhängig davon weiter, ob der Nutzer den Screen wechselt oder die App minimiert – dafür zeigt die System-Statusleiste eine dauerhafte Fortschritts-Benachrichtigung (Dateiname, Phase, Fortschrittsbalken), solange ein Download aktiv ist; sie verschwindet automatisch nach Abschluss. Kehrt der Nutzer während eines laufenden Downloads auf den Screen zurück (oder öffnet ihn neu), zeigt die betroffene Zeile sofort wieder den Fortschritt bzw. nach Abschluss den Installieren/Teilen/Löschen-Dialog. Pro App läuft immer nur ein Download gleichzeitig, unabhängig davon, von welchem Screen aus er gestartet wurde; ein Versuch, während eines laufenden Downloads einen weiteren zu starten, zeigt eine Fehlermeldung statt eines zweiten, parallelen Downloads.
...
- **Kontexte** (optional, Mehrfachauswahl): Ein Filter-Chip pro unter Einstellungen → Kontexte gepflegtem Preset, beliebig viele gleichzeitig antippbar. Sind Kontexte gewählt, werden ihre Werte in der angezeigten Reihenfolge vor den eingegebenen Prompt gehängt (`<Kontext 1>\n\n<Kontext 2>\n\n<Prompt>`), bevor der Befehl an den Server geht.
- **Model** (optional): Dropdown Standard/`haiku`/`sonnet`/`opus`/`fable`, Default vorausgewählt.
- **Prompt**: Mehrzeiliges Textfeld, optional – nur Pflichtfeld, wenn kein Kontext gewählt ist. Sind Kontexte gewählt und der Prompt ist leer, werden nur die Kontext-Werte gesendet.

„Starten" ruft `POST /<agent>` auf und öffnet die Status-Detailseite.

## Dev-Kontexte (Einstellungen)

Unter Einstellungen → Kontexte lassen sich beliebig viele Name+Wert-Presets anlegen, bearbeiten und löschen (lokal in Room gespeichert). Sie dienen ausschließlich dazu, beim Start eines Agenten unter „Entwicklung" vor den Prompt gehängt zu werden (beliebig viele gleichzeitig, siehe oben) – kein Server-seitiges Konzept.

## Command-Status (live)

Abonniert `GET /state/<id>/stream` (Server-Sent Events): Output-Updates kommen dadurch als Push, ohne festes Intervall, sobald der Server sie hat. Bricht die Stream-Verbindung ab, bevor ein Endstatus (`completed`/`failed`/`stopped`) ankam, fällt die App automatisch auf Polling von `GET /state/<id>` alle 2 Sekunden zurück, bis `status != "running"` ist – z. B. wenn ein Proxy zwischen App und `cl server` lang laufende Verbindungen kappt. Zeigt Status-Pill, Agent/Command-Text (daneben ein Icon-Button kopiert den Command-Text/Prompt in die Zwischenablage, bestätigt per Toast „Input kopiert"), den Startzeitpunkt (`createdAt`, Geräte-Zeitzone) sowie die Laufzeit – solange der Command läuft, tickt sie lokal sekündlich hoch (`createdAt` bis aktuelle Gerätezeit), unabhängig davon, ob gerade neuer Output/`updatedAt` vom Server kommt; nach Abschluss friert sie auf `updatedAt - createdAt` ein –, den bisherigen Output live (monospace) und den Exit-Code nach Abschluss. Handelt es sich um einen echten Agenten-/Scheduler-Lauf mit LLM-Aufruf, zeigt eine zusätzliche Zeile die dabei entstandenen Kosten (`costUsd`, vier Nachkommastellen, z. B. „Kosten: $0,0200") sowie direkt darunter eine Zeile mit dem gesamten Tokenverbrauch dieses einen Laufs (Summe aus `inputTokens`/`outputTokens`/`cacheCreationInputTokens`/`cacheReadInputTokens`, mit Tausendertrennzeichen, z. B. „Tokens gesamt: 12.480"); Pfad-Commands, Hooks und Skript-Scheduler haben keine Kosten/Tokens und zeigen beide Zeilen nicht. Der „Output"-Bereich ist einklappbar: Antippen der Titelzeile (Pfeil-Icon zeigt den Zustand) blendet den Output-Text ein/aus, standardmäßig aufgeklappt; der Zustand ist rein lokal für die geöffnete Seite und bleibt nicht über ein Neuöffnen hinweg erhalten. Neben dem „Output"-Titel kopiert ein Icon-Button den kompletten Output in die Zwischenablage (`LocalClipboardManager`), bestätigt per Toast „Output kopiert" – unabhängig vom Ein-/Ausgeklappt-Zustand. Wurde die Seite mit einem Projektnamen geöffnet und hat dieses Projekt `hosted`-Datei-Einträge, erscheinen nach erfolgreichem Abschluss zusätzlich Schnellzugriff-Download-Buttons (inkl. Fortschrittsanzeige, Install-Icon für bereits heruntergeladene APKs und Installieren/Teilen/Löschen-Dialog, siehe „Downloads").
...
Erreichbar über den „Verlauf"-Eintrag im Projekt-Hub, immer auf das aktuelle Projekt beschränkt (`GET /commands/<pathName>?limit=&offset=`). Zeigt alle bisher für dieses Projekt abgeschickten Commands (Agenten-Läufe aus „Entwicklung" **und** Befehle) als Liste – Status-Pill, Agent, ein einklappbares „Befehl"-Feld (standardmäßig eingeklappt, ein Antippen auf das Label/den Pfeil zeigt den vollständigen Command-Text ohne Kürzung, erneutes Antippen klappt ihn wieder ein; der Zustand ist rein lokal pro Zeile und bleibt nicht über Scroll/Neuladen hinweg erhalten), Zeitstempel und Laufzeit, neueste zuerst. Serverseitig in 5er-Schritten paginiert: die Liste laedt anfangs die 5 neuesten Eintraege, ein „Mehr laden"-Button am Ende der Liste haengt jeweils 5 weitere an, solange der Server noch aeltere Eintraege hat. Pollt alle 3 Sekunden automatisch neu, solange der Screen offen ist – aktualisiert dabei nur das aktuell geladene Fenster (nicht wieder nur die ersten 5), sodass bereits nachgeladene Seiten und die Scroll-Position erhalten bleiben; neue Commands (aus „Befehle"/„Entwicklung" gestartet oder per Ticket-Play ausgelöst) erscheinen ohne manuellen Refresh ganz oben, ohne die Liste zurueckzusetzen. Tippen auf einen Eintrag öffnet dieselbe Status-Detailseite wie „Befehle"/„Entwicklung" (Live-Output, Downloads).
...
Der Eintrag „Scheduler" im Projekt-Hub erscheint nur, wenn `GET /paths/<pathName>/schedulers` für das aktuelle Projekt mindestens einen Scheduler oder Script-Scheduler zurückliefert – ein Projekt ohne konfigurierte (Script-)Scheduler zeigt den Eintrag gar nicht erst an. Der Screen selbst lädt dieselbe Liste erneut und zeigt jeden Eintrag (Scheduler wie Script-Scheduler gemeinsam, ohne optische Trennung außer der Reihenfolge) als Zeile mit Name, Beschreibung, Cron-Ausdruck, einem Aktiviert/Deaktiviert-Hinweistext sowie einem Start-Icon. Tippen auf die Zeile selbst öffnet die projektübergreifende Scheduler-Detailseite (siehe „Alle Scheduler" unten); tippen auf das Start-Icon öffnet stattdessen einen Bestätigungsdialog („Scheduler jetzt ausführen?" mit Namen) – erst nach Bestätigung wird `POST /paths/<pathName>/schedulers/<name>/trigger` (Agent-Scheduler) bzw. `POST /paths/<pathName>/script-schedulers/<name>/trigger` (Script-Scheduler) aufgerufen und die Status-Detailseite geöffnet, identisch zum Verhalten bei „Befehle" oben. Während ein Trigger läuft, zeigt die betroffene Zeile statt des Start-Icons einen Ladeindikator.

**Deaktivieren/Aktivieren nur für dieses Projekt:** Jede Zeile zeigt zusätzlich einen Schalter (`enabled`-Feld aus `GET /paths/<pathName>/schedulers`, siehe `../FEATURES.md`, „Scheduler deaktivieren/aktivieren"). Umschalten ruft sofort `POST /paths/<pathName>/schedulers/<name>/enable|disable` bzw. die Script-Scheduler-Variante auf (kein Bestätigungsdialog, da jederzeit rückgängig machbar) – während der Aufruf läuft, ist der Schalter gesperrt; schlägt er fehl, bleibt der bisherige Zustand sichtbar und ein Fehler-Banner erscheint. Ein deaktivierter Scheduler löst nur den **automatischen** `cron`-Zeitplan für dieses eine Projekt nicht mehr aus – der manuelle Start-Button (Play-Icon) bleibt unabhängig davon jederzeit nutzbar, auch bei deaktiviertem Zeitplan. Die Änderung wirkt sofort auf `cl server`, ohne `config.json` zu ändern.

## Alle Scheduler

Projektübergreifende Übersicht aller in `config.json` konfigurierten Scheduler/Script-Scheduler, erreichbar über Einstellungen → „Alle Scheduler" (`GET /schedulers` + `GET /script-schedulers`, siehe `../FEATURES.md`, „Scheduler"/„Script-Scheduler"). Jede Zeile zeigt Name, Beschreibung, Cron-Ausdruck sowie die Liste der zugeordneten Projekte (`paths`, kommagetrennt); ohne jeden konfigurierten (Script-)Scheduler erscheint der Hinweis „Für kein Projekt sind Scheduler konfiguriert.". Tippen auf eine Zeile öffnet die Detailseite.

**Detailseite:** zeigt Name, Beschreibung, Cron-Ausdruck sowie den genauen, pro Lauf tatsächlich verwendeten Ausführungstext – bei einem Agent-Scheduler der aufgelöste Auftragstext (`instructions`, aus dem `scheduler/<name>.md`-Context plus etwaigen zusätzlichen `contexts`, in Monospace dargestellt), bei einem Script-Scheduler stattdessen dessen Bash-Skript (`script`, ebenfalls Monospace). Darunter, zentral an einem Ort statt verteilt über die einzelnen Projekt-Hubs, je ein Eintrag pro zugeordnetem Projekt (`pathStatuses`) mit eigenem Schalter zum Aktivieren/Deaktivieren nur für dieses eine Projekt – Umschalten ruft denselben `POST /paths/<pathName>/schedulers|script-schedulers/<name>/enable|disable`-Endpunkt wie im projektbezogenen Scheduler-Screen auf (siehe oben), während des Aufrufs ist nur der betroffene Schalter gesperrt, ein Fehler-Banner erscheint bei einem fehlgeschlagenen Aufruf und der vorherige Zustand bleibt sichtbar. Dieselbe Detailseite ist auch aus dem projektbezogenen Scheduler-Screen erreichbar (siehe oben).
...
Erreichbar über Einstellungen → System-Metriken – ebenfalls projektübergreifend, zeigt die Systemauslastung der Maschine, auf der `cl server` läuft (`GET /system-metrics`, festes 24-Stunden-Fenster, Samples im Abstand von ca. 5 Minuten). Zwei Karten mit je einem selbstgezeichneten Liniendiagramm (Compose-Canvas, keine Chart-Bibliothek):

- **CPU-Auslastung:** aktueller Wert groß über dem Diagramm, darunter der Verlauf; die CPU-Last basiert auf dem Ein-Minuten-Load-Average geteilt durch die Kernzahl und kann daher über 100 % liegen (Systemüberlastung) – das Diagramm skaliert sich dafür automatisch, ohne den Wert zu kappen.
- **Arbeitsspeicher:** aktueller Wert als „<belegt> / <gesamt> GiB" über dem Diagramm, darunter der Verlauf der Auslastung in Prozent.

Beide Diagramme zeigen zusätzlich Minimum/aktuellen Wert/Maximum als kleine Textwerte unter dem Diagramm; ohne Messwerte (Server gerade erst gestartet) erscheint statt eines leeren Diagramms ein Hinweistext. Lädt einmalig beim Öffnen, ein Button „Aktualisieren" lädt erneut; ein Fehler lässt die zuletzt geladenen Daten unverändert stehen.
