...
Erreichbar über den „Verlauf"-Eintrag im Projekt-Hub, immer auf das aktuelle Projekt beschränkt (`GET /commands/<pathName>?limit=&offset=`). Zeigt alle bisher für dieses Projekt abgeschickten Commands (Agenten-Läufe aus „Entwicklung" **und** Befehle) als Liste – Status-Pill, Agent, Command-Text (gekürzt), Zeitstempel und Laufzeit, neueste zuerst. Serverseitig in 5er-Schritten paginiert: die Liste laedt anfangs die 5 neuesten Eintraege, ein „Mehr laden"-Button am Ende der Liste haengt jeweils 5 weitere an, solange der Server noch aeltere Eintraege hat. Pollt alle 3 Sekunden automatisch neu, solange der Screen offen ist – aktualisiert dabei nur das aktuell geladene Fenster (nicht wieder nur die ersten 5), sodass bereits nachgeladene Seiten und die Scroll-Position erhalten bleiben; neue Commands (aus „Befehle"/„Entwicklung" gestartet oder per Ticket-Play ausgelöst) erscheinen ohne manuellen Refresh ganz oben, ohne die Liste zurueckzusetzen. Tippen auf einen Eintrag öffnet dieselbe Status-Detailseite wie „Befehle"/„Entwicklung" (Live-Output, Downloads).
