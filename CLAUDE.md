...
**Ausnahme:** `commander/` ist ein eigenständiges, verschachteltes Android-Gradle-Projekt (Fernsteuerungs-App für `cl server`) – dort gilt die WaffleHQ-Android-Basisregel wieder, siehe `commander/CLAUDE.md`/`commander/context.md`. Der Rest dieses Repos (alles außer `commander/`) bleibt reines TypeScript.
