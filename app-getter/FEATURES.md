...
Sobald ein Server gefunden ist, listet die App alle über `GET /collections` gemeldeten Dateien auf: Name sowie gut sichtbar „Gebaut am <dd.mm.yyyy hh:mm>" (Zeitpunkt der letzten Änderung der Datei, in die Zeitzone des Geräts umgerechnet), neueste zuerst. Ohne Dateien erscheint ein Leer-Hinweis. Kehrt man aus dem Hintergrund in die App zurück oder öffnet sie erneut, wird die Liste automatisch neu geladen – kein manuelles Aktualisieren nötig.
...
Tippen auf eine noch nicht heruntergeladene Datei lädt sie über `GET /collections/get/<name>` in den App-eigenen Downloads-Ordner. Nach Abschluss erscheint ein Dialog mit drei Aktionen: „Installieren" (`ACTION_VIEW`, MIME `application/vnd.android.package-archive`, über einen eigenen `FileProvider` – Android verlangt weiterhin einmalig die Nutzerbestätigung „Installation aus unbekannten Quellen erlauben", ab API 26 plattformseitig zwingend), „Teilen" (`ACTION_SEND`-Chooser, damit sich die APK z. B. per Messenger/Mail/Cloud-Speicher an andere Geräte weitergeben lässt) oder „Löschen" (Bestätigungsnachfrage, entfernt die Datei vom Gerät). Tippen außerhalb des Dialogs bricht ohne Aktion ab, ohne die Datei zu löschen.

Während des Downloads zeigt die betroffene Dateizeile statt des Download-Icons einen `LinearProgressIndicator` mit Live-Text darunter: Prozentsatz, Download-Geschwindigkeit (B/s, KB/s oder MB/s) und verbleibende Zeit (sofern die Serverantwort eine Content-Length liefert – sonst ein unbestimmter Balken nur mit Geschwindigkeit). Darunter wechselt das Text-Label je Phase: „Wird heruntergeladen…" → „Wird überprüft…" (Datei wird nach Abschluss auf Vollständigkeit geprüft) → „Wird installiert…" (kurz bevor der Installieren/Teilen/Löschen-Dialog erscheint). Die übrigen Zeilen sind während eines laufenden Downloads deaktiviert (nur ein Download gleichzeitig).

Eine fertig heruntergeladene, noch nicht gelöschte Datei bleibt dauerhaft gespeichert (Pfad + Zeitstempel in `DownloadHistoryRepository`, DataStore), die betroffene Zeile zeigt statt des Download- ein Install-Icon. Ein Tap darauf fragt zuerst direkt `GET /collections` ab und vergleicht den aktuellen Server-Zeitstempel der Datei mit dem beim Download gespeicherten. Stimmen sie überein, öffnet sich der Installieren/Teilen/Löschen-Dialog sofort, ohne erneut herunterzuladen. Hat sich die Datei auf dem Server geändert, wird die gecachte Datei sofort vom Gerät gelöscht und automatisch neu heruntergeladen, statt die veraltete Version zur Installation anzubieten. Der Dialog öffnet sich dabei **nicht** von selbst (weder beim Laden der Liste noch beim App-Neustart) – nur ein bewusster Tap auf das Install-/Download-Icon öffnet ihn. Erst „Löschen" entfernt die Datei endgültig vom Gerät; existiert die zwischengespeicherte Datei nicht mehr (z. B. manuell gelöscht), zeigt die Zeile wieder das Download-Icon und ein Tap lädt neu herunter.
...
- **`CollectionsViewModelTest`** – `downloadingFileName`-Zustand während des Downloads (gesetzt bis der Install-Intent konsumiert wird, geleert bei Fehler, zweiter Tap während eines laufenden Downloads wird ignoriert), `downloadStatus`-Durchreichung von `ApkInstaller`, `downloadedTimestamps`-/`pendingInstalls`-Durchreichung von `DownloadHistoryRepository` sowie `recordDownload()`/`recordPendingInstall()` nach erfolgreichem Download, gegen gemockte `ApkInstaller`/`AppGetterApi`/`SettingsRepository`/`DownloadHistoryRepository` (MockK, `StandardTestDispatcher`). Zusätzlich: `pendingInstalls` öffnet nach dem Scan **nicht** automatisch den Dialog; ein Pending-Install auf eine inzwischen fehlende Datei wird beim Scan verworfen (`clearPendingInstall`); ein Tap auf das Install-Icon prüft zuerst den aktuellen Server-Zeitstempel und liefert bei unverändertem Zeitstempel die gecachte Datei ohne erneuten Download der APK selbst; bei geändertem Zeitstempel wird die gecachte Datei gelöscht und neu heruntergeladen; `resolveInstallFile()` (Installieren/Teilen) löscht den Pending-Install-Eintrag, `consumeInstallFile()` (Dismiss) tut das ausdrücklich nicht.
