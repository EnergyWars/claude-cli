...
- **`GET /paths/<pathName>/schedulers`** – liefert `{ schedulers, scriptSchedulers }`, jeweils gefiltert auf Einträge, die diesen `pathName` führen (404, falls `pathName` unbekannt). Siehe Abschnitt "Scheduler (config.json schedulers[], scheduler/<name>.md)".
