...
## Scheduler (`config.json` `schedulers[]`, `scheduler/<name>.md`)

Cron-gesteuerte, headless `claude`-Läufe innerhalb von `cl server` – ein Scheduler feuert in erster Linie automatisch nach seinem konfigurierten Zeitplan, solange der Server läuft; zusätzlich lässt sich jeder Scheduler manuell für ein einzelnes Projekt anstoßen (siehe "Manueller Trigger" unten).

Jeder Eintrag in `config.json`s `schedulers[]` ist strukturell nah an einem Task/Agent (`{ name, description, cron, paths, model, contexts?, permissions? }`), zusätzlich mit zwei Pflichtfeldern:

- `cron` (String) – Cron-Ausdruck in [`croner`](https://github.com/Hexagon/croner)-Syntax (5 Standard-Felder `Minute Stunde Tag Monat Wochentag`, optional ein führendes Sekunden-Feld für 6 Felder, z. B. `"0 0 3 * * *"` für "jeden Tag um 3 Uhr" oder `"*/30 * * * * *"` für "alle 30 Sekunden").
- `paths` (String-Array, nicht leer) – Namen von Einträgen aus `config.json`s `paths[]`; je einer ein eigener headless `claude`-Lauf mit dem zugehörigen Dateisystem-Pfad als `cwd`, genau wie bei `POST /`/`POST /<agent>`.

**Eigener Context pro Scheduler:** im Projektstamm liegt ein Unterordner `scheduler/`, darin für jeden konfigurierten Scheduler eine `<name>.md`-Datei (z. B. `schedulers[].name === "nightly-sync"` → `scheduler/nightly-sync.md`) – der eigentliche Auftrag, den dieser Lauf jedes Mal automatisch bekommt. Diese Datei wird **immer zuerst** in den System-Prompt aufgenommen. Zusätzlich kann (wie bei Agents/Tasks) ein optionales `contexts`-Array beliebige weitere, gemeinsam genutzte Dateien aus `contexts/` referenzieren – deren Inhalt wird danach angehängt. `scheduler/*.md`-Dateien werden genau wie `contexts/*.md` beim Build fest ins Binary eingebettet (`scripts/generate-context-bundle.mjs` → `EMBEDDED_SCHEDULER_CONTEXTS`), lokal-first mit Embedded-Fallback (`resolveSchedulerContext(name)` in `src/config.ts`) – funktioniert also identisch zum bestehenden Config/Context-System, auch im deployten Einzeldatei-Binary ohne begleitendes `scheduler/`-Verzeichnis.

**Ablauf eines Triggers:** sobald der Cron-Ausdruck eines Schedulers matcht, läuft exakt derselbe Mechanismus wie bei `POST /`/`POST /<agent>` – eine `id` wird generiert, sofort eine `t_commands`-Zeile mit `status: "running"` und `agent: "scheduler:<name>"` angelegt, danach `claude --model <model> --append-system-prompt <scheduler-Context + contexts> --permission-mode auto --print "<fester Trigger-Text>"` im Hintergrund gestartet (`cwd` = aufgelöster Dateisystem-Pfad). Output läuft live in `t_commands.output` (SSE über `GET /state/<id>/stream` funktioniert identisch), Status wechselt am Ende auf `"completed"`/`"failed"`/`"stopped"`. Ein Scheduler-Lauf ist damit **ununterscheidbar** von einem normalen Agent-Lauf in `GET /commands/<pathName>`/`GET /state/<id>` – nur der `agent`-Wert (`"scheduler:<name>"` statt eines Agent-Namens) verrät die Herkunft. Läuft in demselben Pfad kein anderer Agent mehr, feuert danach auch der `onLastAgentFinish`-Hook dieses Pfads (siehe "Pfad-Hooks (`paths[].hooks`)"), genau wie nach jedem anderen Agent-Lauf.

**`GET /schedulers`** (authentifiziert) listet alle konfigurierten Scheduler projektübergreifend als `{ schedulers: [{ name, description, cron, paths, instructions, pathStatuses }] }` – ohne Permissions. `instructions` ist der pro Lauf tatsächlich aufgelöste Auftragstext (`buildSchedulerSystemPrompt(scheduler)`, also `scheduler/<name>.md` plus etwaige zusätzliche `contexts`) statt eines Fehlertexts (`"Auftragstext konnte nicht aufgeloest werden: ..."`), falls sich der Context nicht auflösen lässt. `pathStatuses` ist `[{ pathName, enabled }]` je Eintrag aus `paths` – der DB-gestützte Enabled-Status (siehe "Scheduler deaktivieren/aktivieren") für **alle** zugeordneten Projekte in einem Aufruf, Grundlage für eine zentrale, projektübergreifende Scheduler-Übersicht (siehe `commander/FEATURES.md`). Die (unangereicherte) Liste ohne `instructions`/`pathStatuses` taucht weiterhin unter `schedulers` in `GET /manifest` auf (siehe "Manifest (`GET /manifest`)"). Ein ungültiger Cron-Ausdruck oder ein unbekannter `paths`-Name lässt den Server beim automatischen Trigger nicht abstürzen: der betroffene Scheduler wird beim Start übersprungen bzw. der einzelne Pfad schlägt fehl (jeweils mit `console.error`-Meldung), alle anderen Scheduler/Pfade laufen unbeeinflusst weiter.

**Manueller Trigger, beschränkt auf ein Projekt (`GET /paths/:pathName/schedulers`, `POST /paths/:pathName/schedulers/:name/trigger`):** zusätzlich zum automatischen `cron`-Zeitplan lässt sich jeder Scheduler auch gezielt für **ein einzelnes** Projekt einmalig anstoßen, ohne dabei die übrigen in `schedulers[].paths` konfigurierten Pfade mit anzustoßen. `GET /paths/:pathName/schedulers` (authentifiziert, 404 bei unbekanntem `pathName`) liefert `{ schedulers: [...], scriptSchedulers: [...] }`, jeweils gefiltert auf Einträge, deren `paths[]`-Array diesen `pathName` enthält – Grundlage dafür, in einer UI (z. B. `commander`) nur dann eine Trigger-Möglichkeit anzuzeigen, wenn das ausgewählte Projekt überhaupt Scheduler/Script-Scheduler führt. `POST /paths/:pathName/schedulers/:name/trigger` (kein Body) läuft intern über denselben `triggerSchedulerForPath()`-Mechanismus wie ein automatischer Cron-Treffer (derselbe `t_commands`-Eintrag mit `agent: "scheduler:<name>"`, dasselbe Live-Output über `GET /state/<id>/stream`, derselbe anschließende `onLastAgentFinish`-Hook) und antwortet sofort mit `202 { id }`; `404`, falls `pathName` oder der Scheduler-`name` unbekannt sind, `400`, falls dieser Scheduler den angegebenen `pathName` gar nicht in seinem `paths[]`-Array führt.

**Hot-Reload:** ändert sich `schedulers[]` über `PUT /config` oder `PUT /config/pointer` (siehe "Config-Editierung + Versionshistorie"), werden alle bisher laufenden Cron-Jobs sofort gestoppt und aus der neuen Config neu registriert – ohne Server-Neustart.

Implementiert in `src/scheduler.ts` (`startSchedulers(schedulers, onTrigger)` – dünner Wrapper um [`croner`](https://www.npmjs.com/package/croner) (MIT-lizenziert, keine Abhängigkeiten), ein `Cron`-Job pro Scheduler, `stop()` beendet alle Jobs), `src/launch.ts` (`buildSchedulerSystemPrompt(scheduler)`, `SCHEDULER_TRIGGER_PROMPT`), `src/config.ts` (`SchedulerDefinition`/`SchedulerConfig`, `resolveSchedulerContext`, `listSchedulers`, `listSchedulersForPath`, `resolveScheduler`), `src/server.ts` (`triggerScheduler`/`triggerSchedulerForPath` (gibt die Command-ID zurück), `handleGetSchedulers` (reichert per `resolveSchedulerPathStatuses()` um `instructions`/`pathStatuses` an), `handleGetPathSchedulers`, `handlePostSchedulerTrigger`, `SchedulerState`/`restartSchedulers` für den Hot-Reload), `scripts/generate-context-bundle.mjs`, `openapi.json`.

## Script-Scheduler (`config.json` `scriptSchedulers[]`)

Cron-gesteuerte Bash-Läufe innerhalb von `cl server`, ergänzend zu den (Agent-)Schedulern oben: statt eines `claude`-Agenten wird ein reiner Shell-Befehl ausgeführt – gedacht für Wartungsaufgaben wie automatisches Committen/Pushen, ohne dafür ein LLM zu bemühen. Wie Scheduler feuert ein Script-Scheduler in erster Linie automatisch nach seinem konfigurierten Zeitplan, solange der Server läuft; ein manueller Trigger für ein einzelnes Projekt ist ebenfalls möglich (siehe "Manueller Trigger" unten).

Jeder Eintrag in `config.json`s optionalem `scriptSchedulers[]` ist `{ name, description, cron, paths, script }` – kein `model`/`contexts`/`permissions`, da kein `claude`-Prozess involviert ist:

- `cron` (String) – Cron-Ausdruck in `croner`-Syntax, identisch zu `schedulers[].cron`.
- `paths` (String-Array, nicht leer) – Namen aus `config.json`s `paths[]`; je einer ein eigener Lauf mit diesem Verzeichnis als `cwd`.
- `script` (String) – Bash-Befehl, per `spawn(script, { shell: true, cwd })` ausgeführt (identischer Mechanismus wie `paths[].commands`/`paths[].hooks.onLastAgentFinish`), z. B. `"bash scripts/auto-commit.sh"`.

**Ablauf eines Triggers:** sobald der Cron-Ausdruck matcht, wird für jeden Pfad in `paths` sofort eine `t_commands`-Zeile mit `status: "running"`, `agent: "script-scheduler:<name>"` und `model: "-"` angelegt (wie ein Pfad-Command), danach `script` als Shell-Befehl gestartet. Output läuft live in `t_commands.output` (SSE über `GET /state/<id>/stream`), Status wechselt am Ende auf `"completed"`/`"failed"`/`"stopped"`. Ein Script-Scheduler-Lauf zählt **nicht** als Agent-Lauf (`agent NOT LIKE 'script-scheduler:%'` zusätzlich zu `path-command:`/`hook:` in `countRunningAgents`/`countAgentsSince`, siehe "Projekt-Statistik") und löst **nicht** den `onLastAgentFinish`-Hook des Pfads aus (wie Pfad-Commands, nicht wie Agent-/Scheduler-Läufe).

**`GET /script-schedulers`** (authentifiziert) listet alle konfigurierten Script-Scheduler projektübergreifend als `{ scriptSchedulers: [{ name, description, cron, paths, script, pathStatuses }] }`. `pathStatuses` ist `[{ pathName, enabled }]` je Eintrag aus `paths`, analog zu `GET /schedulers` oben. Die (unangereicherte) Liste ohne `pathStatuses` taucht weiterhin unter `scriptSchedulers` in `GET /manifest` auf (siehe "Manifest (`GET /manifest`)"). Ein ungültiger Cron-Ausdruck oder ein unbekannter `paths`-Name lässt den Server beim automatischen Trigger nicht abstürzen (der betroffene Script-Scheduler bzw. Pfad wird übersprungen, `console.error`-Meldung, die übrigen laufen weiter).

**Manueller Trigger, beschränkt auf ein Projekt (`GET /paths/:pathName/schedulers`, `POST /paths/:pathName/script-schedulers/:name/trigger`):** analog zu Schedulern oben – `GET /paths/:pathName/schedulers` liefert unter `scriptSchedulers` nur die Einträge, die diesen `pathName` in ihrem `paths[]`-Array führen (siehe "Scheduler" oben, derselbe Endpunkt für beide Typen). `POST /paths/:pathName/script-schedulers/:name/trigger` (kein Body) startet das Script nur für diesen einen Pfad über denselben `triggerScriptSchedulerForPath()`-Mechanismus wie ein automatischer Cron-Treffer (`t_commands`-Eintrag `agent: "script-scheduler:<name>"`, `model: "-"`, kein `onLastAgentFinish`-Hook, kein Einfluss auf die Agenten-Statistik) und antwortet sofort mit `202 { id }`; `404` bei unbekanntem `pathName`/`name`, `400`, falls dieser Script-Scheduler den `pathName` nicht führt.

**Hot-Reload:** ändert sich `scriptSchedulers[]` über `PUT /config` oder `PUT /config/pointer`, werden alle bisher laufenden Cron-Jobs sofort gestoppt und aus der neuen Config neu registriert – ohne Server-Neustart (derselbe `restartSchedulers()`-Aufruf wie bei `schedulers[]`, siehe "Config-Editierung + Versionshistorie").

**Beispiel-Konfiguration (`config.json`):** ein Eintrag `auto-commit-hourly` (Cron `"0 * * * *"`, jede volle Stunde) führt `scripts/auto-commit.sh` in den Pfaden `periodical` und `carfinder` aus. Das Script (`git add .` → bei leerem `git diff --cached` No-op-Exit `0`, sonst `git commit -m "Auto commit <Datum> <Uhrzeit>"` + `git push`) committet/pusht also nur, wenn es tatsächlich Änderungen gibt.

Implementiert in `src/scheduler.ts` (`startSchedulers<T>` – generischer Wrapper, seit diesem Feature per Typparameter sowohl für `SchedulerConfig` als auch `ScriptSchedulerConfig` nutzbar, statt fest auf `SchedulerConfig` typisiert), `src/config.ts` (`ScriptSchedulerDefinition`/`ScriptSchedulerConfig`, `listScriptSchedulers`, `listScriptSchedulersForPath`, `resolveScriptScheduler`), `src/server.ts` (`triggerScriptScheduler`/`triggerScriptSchedulerForPath` (gibt die Command-ID zurück) – nutzen `runShellCommand()` aus `src/launch.ts` wie Pfad-Commands, `handleGetScriptSchedulers`, `handleGetPathSchedulers`, `handlePostScriptSchedulerTrigger`, `SchedulerState.scriptRegistry`/`restartSchedulers` für den Hot-Reload), `src/db.ts` (`AGENT_COMMANDS_ONLY_CLAUSE` um `script-scheduler:%` ergänzt), `scripts/auto-commit.sh`, `openapi.json`.

## Scheduler deaktivieren/aktivieren (`POST /paths/<pathName>/schedulers/<name>/enable|disable`, `POST /paths/<pathName>/script-schedulers/<name>/enable|disable`)

Jeder Scheduler und Script-Scheduler lässt sich pro Projekt einzeln deaktivieren, ohne `config.json` anzufassen – die Änderung landet ausschließlich in der Datenbank (Tabelle `t_scheduler_disabled`). Ein deaktivierter Eintrag löst **nur beim automatischen Cron-Zeitplan** für dieses eine Projekt nicht mehr aus; die übrigen in `schedulers[].paths`/`scriptSchedulers[].paths` konfigurierten Projekte desselben Schedulers bleiben unbeeinflusst, und ein manueller Trigger (`POST .../trigger`) bleibt trotz Deaktivierung weiterhin möglich.

- **`POST /paths/<pathName>/schedulers/<name>/disable`** / **`POST /paths/<pathName>/schedulers/<name>/enable`** – deaktiviert bzw. aktiviert einen Scheduler nur für `pathName` (kein Body). `404`, falls `pathName`/`name` unbekannt ist; `400`, falls dieser Scheduler `pathName` gar nicht in seinem `paths[]`-Array führt. Antwort bei Erfolg: `200 { "name": "<name>", "pathName": "<pathName>", "enabled": true|false }`.
- **`POST /paths/<pathName>/script-schedulers/<name>/disable`** / **`.../enable`** – analog für Script-Scheduler.
- **`GET /paths/<pathName>/schedulers`** liefert pro Eintrag zusätzlich `enabled: boolean` (Default `true`, solange nichts deaktiviert wurde) – sowohl unter `schedulers` als auch unter `scriptSchedulers`, bezogen auf genau diesen einen Pfad. **`GET /schedulers`/`GET /script-schedulers`** (die projektübergreifenden Listen) liefern denselben Status für **alle** zugeordneten Pfade gleichzeitig als `pathStatuses: [{ pathName, enabled }]` (siehe oben) – `GET /manifest` enthält weder `enabled` noch `pathStatuses`, da dort bewusst nur die unangereicherte Konfigurationssicht steht.
- Aktivieren löscht den Deaktivierungs-Eintrag wieder (statt ein Flag umzuschalten) – der Standardzustand ohne jede Deaktivierung ist also immer "aktiviert", auch nach einer DB-Migration auf eine ältere `cl`-Version ohne dieses Feature.

Implementiert in `src/db.ts` (Tabelle `t_scheduler_disabled` mit Primärschlüssel `(kind, name, path_name)`, `SchedulerKind = "scheduler" | "script-scheduler"`, `setSchedulerEnabled(db, kind, name, pathName, enabled)`, `isSchedulerDisabled(db, kind, name, pathName)`), `src/server.ts` (`handlePostSchedulerEnabled()` – gemeinsamer Handler für beide Typen und beide Richtungen, `handleGetPathSchedulers()` reichert die Listen um `enabled` an, `resolveSchedulerPathStatuses()` macht dasselbe projektübergreifend für `handleGetSchedulers()`/`handleGetScriptSchedulers()`, `triggerScheduler()`/`triggerScriptScheduler()` überspringen beim automatischen Cron-Trigger einen deaktivierten Pfad per `isSchedulerDisabled()`-Check – der manuelle Trigger-Handler prüft das bewusst nicht), `openapi.json` (`PathScheduler`/`PathScriptScheduler`/`SchedulerEnabledUpdate`/`SchedulerPathStatus`/`SchedulerWithDetails`/`ScriptSchedulerWithDetails`-Schemas, vier neue Pfade). `commander` zeigt den Zustand pro Scheduler und Projekt zentral in einer projektübergreifenden Scheduler-Übersicht an (siehe `commander/FEATURES.md`).
...
- **`GET /manifest`** – liefert Agents, Pfade (inkl. deren Commands/Hosted-Einträgen) und Scheduler gebündelt in einem Aufruf; enthält **keine** Tasks (siehe "Task-Ausführung (cl task <name>)"). Siehe Abschnitt "Manifest (GET /manifest)".
- **`GET /schedulers`** – liefert alle konfigurierten Scheduler projektübergreifend (`{ name, description, cron, paths, instructions, pathStatuses }`). Siehe Abschnitt "Scheduler (config.json schedulers[], scheduler/<name>.md)".
- **`GET /script-schedulers`** – liefert alle konfigurierten Script-Scheduler projektübergreifend (`{ name, description, cron, paths, script, pathStatuses }`). Siehe Abschnitt "Script-Scheduler (config.json scriptSchedulers[])".
- **`GET /paths/<pathName>/schedulers`** – liefert `{ schedulers, scriptSchedulers }` (jeweils inkl. `enabled: boolean` pro Eintrag), gefiltert auf Einträge, die diesen `pathName` führen (404, falls `pathName` unbekannt). Siehe Abschnitt "Scheduler (config.json schedulers[], scheduler/<name>.md)".
- **`POST /paths/<pathName>/schedulers/<name>/trigger`** / **`POST /paths/<pathName>/script-schedulers/<name>/trigger`** – stoßen den jeweiligen (Script-)Scheduler manuell **nur für dieses eine Projekt** an (404, falls `pathName`/`name` unbekannt; 400, falls dieser (Script-)Scheduler den `pathName` nicht führt). Siehe Abschnitte "Scheduler"/"Script-Scheduler".
- **`POST /paths/<pathName>/schedulers/<name>/enable|disable`** / **`POST /paths/<pathName>/script-schedulers/<name>/enable|disable`** – (de)aktiviert den automatischen Cron-Trigger dieses (Script-)Schedulers **nur für dieses eine Projekt**, in der DB statt in `config.json` (404/400 wie beim Trigger-Endpunkt). Siehe Abschnitt "Scheduler deaktivieren/aktivieren".
...
```json
{
  ...
  "schedulers": [{ "name": "nightly-sync", "description": "...", "cron": "0 0 3 * * *", "paths": ["myapp"] }]
}
```
...
- `schedulers` – die unangereicherte Konfigurationssicht `{ name, description, cron, paths }` (ohne Permissions, ohne `instructions`/`pathStatuses` – im Gegensatz zu `GET /schedulers`). Siehe "Scheduler (config.json schedulers[], scheduler/<name>.md)".
- `scriptSchedulers` – die unangereicherte Konfigurationssicht `{ name, description, cron, paths, script }` (ohne `pathStatuses` – im Gegensatz zu `GET /script-schedulers`). Siehe "Script-Scheduler (config.json scriptSchedulers[])".
...
Implementiert in `src/config.ts` (`listHostedSummaries`, `listSchedulers`) und `src/server.ts` (`handleGetManifest`).
...
- `schedulers` – ein Array benannter Objekte `{ name: string, description: string, cron: string, paths: string[], model: string, contexts?: string[], permissions?: string[] }`, cron-gesteuerte headless `claude`-Läufe innerhalb von `cl server` (siehe "Scheduler (config.json schedulers[], scheduler/<name>.md)") – läuft primär nach `cron`, lässt sich zusätzlich über `POST /paths/:pathName/schedulers/:name/trigger` manuell für ein einzelnes Projekt anstoßen.
- `scriptSchedulers` – optionales Array benannter Objekte `{ name: string, description: string, cron: string, paths: string[], script: string }`, cron-gesteuerte Bash-Läufe innerhalb von `cl server`, analog zu `schedulers` aber ohne `claude`-Agenten (siehe "Script-Scheduler (config.json scriptSchedulers[])") – ebenfalls zusätzlich über `POST /paths/:pathName/script-schedulers/:name/trigger` manuell für ein einzelnes Projekt anstoßbar.
...
Jeder Scheduler bekommt zusätzlich automatisch seinen eigenen, dedizierten Context aus dem Unterordner `scheduler/` (Name 1:1, z. B. `schedulers[].name === "nightly-sync"` → `scheduler/nightly-sync.md`) – siehe "Scheduler (config.json schedulers[], scheduler/<name>.md)".

Aufgelöst über `src/config.ts` (`loadConfig()`, `resolveAgent(name)`, `resolveContext(name)`, `resolveSchedulerContext(name)`):
...
4. Der zu verwendende `adb`-Pfad wird einmalig ermittelt: liegt `adb` in einem `PATH`-Verzeichnis, wird einfach `adb` verwendet; andernfalls wird der Reihe nach `$ANDROID_HOME/platform-tools/adb`, `$ANDROID_SDK_ROOT/platform-tools/adb` und ein plattformabhängiges Standard-SDK-Verzeichnis geprüft (Linux `~/Android/Sdk`, macOS `~/Library/Android/sdk`, Windows `%LOCALAPPDATA%\Android\Sdk`) – der erste gefundene Pfad gewinnt. Das verhindert `spawn adb ENOENT`, wenn zwar das Android SDK installiert ist, `adb` aber nicht im `PATH` liegt. Mit diesem Pfad listet `adb devices` alle aktuell verbundenen Geräte-Serials.
...
Implementiert in `src/gradle-install.ts` (`buildAndInstall`, `findApk`, `parseAdbDevices`, `formatInstallSummary`, `resolveAdbExecutable`, intern `runGradleTask`/`runTaskWithFixLoop`/`runFixAgent`/`hasWarnings`/`listAdbDevices`/`readDeviceName`/`installApk`), registriert als zwei `program.command(...)` in `src/index.ts`. Getestet über echte temporäre Verzeichnisse (`findApk`, `resolveAdbExecutable` mit manipuliertem `PATH`/`ANDROID_HOME`/`ANDROID_SDK_ROOT`/`HOME`), ein Fake-`adb`-Shellscript fürs `PATH` (`src/test-support/mock-adb.ts`), ein skriptbares Fake-`gradlew`-Shellscript im jeweiligen Test-Arbeitsverzeichnis (`src/test-support/mock-gradlew.ts`, liefert pro Aufruf ein anderes Ergebnis aus einer vorgegebenen Schritt-Sequenz) sowie das Fake-`claude`-Binary (`src/test-support/mock-claude.ts`, optionales `logFile` protokolliert Aufrufe auch bei `stdio: 'inherit'`) – sowohl auf Funktions- als auch auf voller CLI-Subprozess-Ebene (`src/gradle-install.test.ts`, `src/index.test.ts`).
...
- **`src/scheduler.test.ts`** – `startSchedulers` gegen echte, sekundengenaue Cron-Ausdrücke (`* * * * * *`): ruft `onTrigger` bei jedem Treffer auf, `stop()` beendet alle Jobs, ein ungültiger Cron-Ausdruck wird übersprungen ohne die übrigen Scheduler zu beeinträchtigen, leere Scheduler-Liste ist ein No-op; zusätzlich ein Test, der die generische Signatur (`startSchedulers<T>`) mit einer von `SchedulerConfig` abweichenden Objektform (z. B. einem Script-Scheduler-artigen `{ name, cron, script }`) belegt.
- **`src/db.test.ts`** – SQLite-Operationen (`openDatabase`, `insertCommand`/`getCommand`, `updateCommandOutput`, `completeCommand`, `listCommands` (Pfad-Filter, neueste zuerst), `logAccess`, `getTotpSecret`/`setPendingTotpSecret`/`confirmTotpSecret`/`deleteTotpSecret`, `insertTicket`/`getTicket`/`listTickets`/`listAllTickets`/`updateTicket`/`deleteTicket` inkl. Status-Filter und pfaduebergreifender ID-Eindeutigkeit, `insertFeedback`/`getFeedback`/`listFeedback` (inkl. optionalem `pathName`-Filter)/`updateFeedback`/`deleteFeedback` inkl. optionalem `section`/`path`, sowie `migrateLegacyTicketColumns` und die `section`-/`path`-Spaltenmigration einer alten `t_feedback`-Tabelle gegen handgebaute Alt-Schema-DBs) gegen echte temporäre `.db`-Dateien; `countRunningAgents`/`countAgentsSince` schließen zusätzlich `script-scheduler:%`-Einträge aus (wie `path-command:%`/`hook:%`).
