...
- `onLastAgentFinish` – ein beliebiger Bash-Befehl. Wird ausgeführt, sobald ein Agent-Lauf (`POST /` bzw. `POST /<agent>`) in diesem Pfad beendet ist (egal ob erfolgreich, fehlgeschlagen oder gestoppt) **und** danach kein weiterer Agent-Lauf in diesem Pfad mehr `running` ist – also genau beim letzten Agenten, der in diesem Pfad fertig wird. Läuft im Dateisystem-Verzeichnis des Pfad-Eintrags (`paths[].path`), fire-and-forget: kein `202`/`id`, keine `GET /state/<id>`-Nachverfolgung, kein Eintrag in `t_commands`. Ein Fehlschlag des Hook-Befehls wird nur serverseitig geloggt (`console.error`) und hat keinen Einfluss auf die Antwort des auslösenden Agent-Laufs. Pfad-Commands (`POST /paths/<pathName>/commands/<key>`) lösen den Hook **nicht** aus – nur echte Agent-Läufe zählen (dieselbe Abgrenzung wie bei `countRunningAgents`, siehe "Projekt-Statistik").

Implementiert in `src/config.ts` (`PathHooks`), `src/server.ts` (`triggerOnLastAgentFinishHook`, aufgerufen am Ende von `handlePostCommand`s Abschluss-Handlern).
