...
Cron-gesteuerte, headless `claude`-Läufe innerhalb von `cl server` – ein Scheduler wird nicht wie ein Agent manuell angestoßen (weder CLI noch API-Request), sondern feuert automatisch nach seinem konfigurierten Zeitplan, solange der Server läuft.

Jeder Eintrag in `config.json`s `schedulers[]` ist strukturell nah an einem Task/Agent (`{ name, description, cron, path, model, contexts?, permissions? }`), zusätzlich mit zwei Pflichtfeldern:
...
- `path` (String) – Name eines Eintrags aus `config.json`s `paths[]`; der zugehörige Dateisystem-Pfad wird als `cwd` des `claude`-Prozesses verwendet, genau wie bei `POST /`/`POST /<agent>`.
...
**`GET /schedulers`** (authentifiziert) listet alle konfigurierten Scheduler als `{ schedulers: [{ name, description, cron, path }] }` – ohne Contexts/Permissions. Zusätzlich taucht dieselbe Liste unter `schedulers` in `GET /manifest` auf (siehe "Manifest (`GET /manifest`)").

**Kein manueller Trigger über CLI/API** – anders als Tasks/Agents lässt sich ein Scheduler nicht gezielt einmalig anstoßen, er läuft ausschließlich nach seinem `cron`-Zeitplan. Ein ungültiger Cron-Ausdruck oder ein unbekannter `path`-Name lässt den Server nicht abstürzen: der betroffene Scheduler wird beim Start übersprungen bzw. der einzelne Trigger schlägt fehl (jeweils mit `console.error`-Meldung), alle anderen Scheduler laufen unbeeinflusst weiter.
...
Implementiert in `src/scheduler.ts` (`startSchedulers(schedulers, onTrigger)` – dünner Wrapper um [`croner`](https://www.npmjs.com/package/croner) (MIT-lizenziert, keine Abhängigkeiten), ein `Cron`-Job pro Scheduler, `stop()` beendet alle Jobs), `src/launch.ts` (`buildSchedulerSystemPrompt(scheduler)`, `SCHEDULER_TRIGGER_PROMPT`), `src/config.ts` (`SchedulerDefinition`/`SchedulerConfig`, `resolveSchedulerContext`, `listSchedulers`), `src/server.ts` (`triggerScheduler`, `handleGetSchedulers`, `SchedulerState`/`restartSchedulers` für den Hot-Reload), `scripts/generate-context-bundle.mjs`.

## Script-Scheduler (`config.json` `scriptSchedulers[]`)

Cron-gesteuerte Bash-Läufe innerhalb von `cl server`, ergänzend zu den (Agent-)Schedulern oben: statt eines `claude`-Agenten wird ein reiner Shell-Befehl ausgeführt – gedacht für Wartungsaufgaben wie automatisches Committen/Pushen, ohne dafür ein LLM zu bemühen. Wie Scheduler wird ein Script-Scheduler nicht manuell angestoßen (weder CLI noch API-Request), sondern feuert automatisch nach seinem konfigurierten Zeitplan, solange der Server läuft.
...
**`GET /script-schedulers`** (authentifiziert) listet alle konfigurierten Script-Scheduler als `{ scriptSchedulers: [{ name, description, cron, paths, script }] }`. Zusätzlich taucht dieselbe Liste unter `scriptSchedulers` in `GET /manifest` auf (siehe "Manifest (`GET /manifest`)").

**Kein manueller Trigger über CLI/API**, wie bei Schedulern – ein ungültiger Cron-Ausdruck oder ein unbekannter `paths`-Name lässt den Server nicht abstürzen (der betroffene Script-Scheduler bzw. Pfad wird übersprungen, `console.error`-Meldung, die übrigen laufen weiter).
...
Implementiert in `src/scheduler.ts` (`startSchedulers<T>` – generischer Wrapper, seit diesem Feature per Typparameter sowohl für `SchedulerConfig` als auch `ScriptSchedulerConfig` nutzbar, statt fest auf `SchedulerConfig` typisiert), `src/config.ts` (`ScriptSchedulerDefinition`/`ScriptSchedulerConfig`, `listScriptSchedulers`), `src/server.ts` (`triggerScriptScheduler`/`triggerScriptSchedulerForPath` – nutzen `runShellCommand()` aus `src/launch.ts` wie Pfad-Commands, `handleGetScriptSchedulers`, `SchedulerState.scriptRegistry`/`restartSchedulers` für den Hot-Reload), `src/db.ts` (`AGENT_COMMANDS_ONLY_CLAUSE` um `script-scheduler:%` ergänzt), `scripts/auto-commit.sh`.
...
- **`GET /schedulers`** – liefert alle konfigurierten Scheduler (`{ name, description, cron, path }`). Siehe Abschnitt "Scheduler (config.json schedulers[], scheduler/<name>.md)".
...
```json
{
  ...
  "schedulers": [{ "name": "nightly-sync", "description": "...", "cron": "0 0 3 * * *", "path": "myapp" }]
}
```
...
- `schedulers` – identisch zu `GET /schedulers` (`{ name, description, cron, path }`, ohne Contexts/Permissions). Siehe "Scheduler (config.json schedulers[], scheduler/<name>.md)".
...
- `schedulers` – ein Array benannter Objekte `{ name: string, description: string, cron: string, path: string, model: string, contexts?: string[], permissions?: string[] }`, cron-gesteuerte headless `claude`-Läufe innerhalb von `cl server` (siehe "Scheduler (config.json schedulers[], scheduler/<name>.md)") – nie manuell über CLI/API auslösbar.
- `scriptSchedulers` – optionales Array benannter Objekte `{ name: string, description: string, cron: string, paths: string[], script: string }`, cron-gesteuerte Bash-Läufe innerhalb von `cl server`, analog zu `schedulers` aber ohne `claude`-Agenten (siehe "Script-Scheduler (config.json scriptSchedulers[])") – ebenfalls nie manuell über CLI/API auslösbar.
