...
**`GET /schedulers`** (authentifiziert) listet alle konfigurierten Scheduler als `{ schedulers: [{ name, description, cron, paths }] }` – ohne Contexts/Permissions. Zusätzlich taucht dieselbe Liste unter `schedulers` in `GET /manifest` auf (siehe "Manifest (`GET /manifest`)"). Ein ungültiger Cron-Ausdruck oder ein unbekannter `paths`-Name lässt den Server beim automatischen Trigger nicht abstürzen: der betroffene Scheduler wird beim Start übersprungen bzw. der einzelne Pfad schlägt fehl (jeweils mit `console.error`-Meldung), alle anderen Scheduler/Pfade laufen unbeeinflusst weiter.
...
**`GET /script-schedulers`** (authentifiziert) listet alle konfigurierten Script-Scheduler als `{ scriptSchedulers: [{ name, description, cron, paths, script }] }`. Zusätzlich taucht dieselbe Liste unter `scriptSchedulers` in `GET /manifest` auf (siehe "Manifest (`GET /manifest`)"). Ein ungültiger Cron-Ausdruck oder ein unbekannter `paths`-Name lässt den Server beim automatischen Trigger nicht abstürzen (der betroffene Script-Scheduler bzw. Pfad wird übersprungen, `console.error`-Meldung, die übrigen laufen weiter).
...
- **`GET /paths/<pathName>/schedulers`** liefert seit diesem Feature pro Eintrag zusätzlich `enabled: boolean` (Default `true`, solange nichts deaktiviert wurde) – sowohl unter `schedulers` als auch unter `scriptSchedulers`. `GET /schedulers`/`GET /script-schedulers` (die pfad-übergreifenden Listen) sowie `GET /manifest` enthalten `enabled` bewusst **nicht**, da der Zustand immer pfadspezifisch ist.
...
- **`GET /schedulers`** – liefert alle konfigurierten Scheduler (`{ name, description, cron, paths }`). Siehe Abschnitt "Scheduler (config.json schedulers[], scheduler/<name>.md)".
- **`GET /script-schedulers`** – liefert alle konfigurierten Script-Scheduler (`{ name, description, cron, paths, script }`). Siehe Abschnitt "Script-Scheduler (config.json scriptSchedulers[])".
...
- `schedulers` – identisch zu `GET /schedulers` (`{ name, description, cron, paths }`, ohne Contexts/Permissions). Siehe "Scheduler (config.json schedulers[], scheduler/<name>.md)".
- `scriptSchedulers` – identisch zu `GET /script-schedulers` (`{ name, description, cron, paths, script }`). Siehe "Script-Scheduler (config.json scriptSchedulers[])".
