...
- **`src/scheduler.test.ts`** – `startSchedulers` gegen echte, sekundengenaue Cron-Ausdrücke (`* * * * * *`): ruft `onTrigger` bei jedem Treffer auf, `stop()` beendet alle Jobs, ein ungültiger Cron-Ausdruck wird übersprungen ohne die übrigen Scheduler zu beeinträchtigen, leere Scheduler-Liste ist ein No-op.
